...
async function main() {
  ...
  program
    .command("install")
    .description(
      "Install code-stick (opencode + Ollama + a coding model) onto a USB drive",
    )
    .option(
      "-t, --target <path>",
      "Install to this directory (skips USB picker)",
    )
    .option(
      "-m, --model <id>",
      "Pick a model non-interactively. Curated ids: qwen25-coder-7b, qwen25-coder-14b, qwen25-coder-32b, deepseek-coder-6_7b, deepseek-coder-v2-16b, codegemma-7b, phi3-mini. Or add any Ollama tag later with `code-stick add-model <tag>`.",
    )
    .option("-y, --yes", "Skip confirmations where possible")
    .option(
      "--no-cleanup",
      "Keep installer archives + temp dirs after install (debugging)",
    )
    .option(
      "--targets <list>",
  ...
  program
    .command("start")
    .description(
      "Start Ollama from the USB and launch opencode in this terminal",
    )
    .option(
      "-t, --target <path>",
      "Use this installation directory (skips picker)",
    )
    .action(async (opts) => {
  ...
  program
    .command("doctor")
    .description(
      "Live audit of an installed stick (filesystem, port, ollama, opencode, models)",
    )
    .option("-t, --target <path>", "Audit this installation directory")
    .option("--status", "Show an on-disk storage breakdown only")
    .option(
      "--no-probe",
      "Skip live ollama serve / opencode --version probes (static checks only)",
    )
    .action(async (opts) => {
  ...
  program
    ...
    .description(
      "Re-download + swap Ollama + opencode binaries, preserving the model store",
    )
    .option("-t, --target <path>", "Upgrade this installation directory")
    .option("-y, --yes", "Skip confirmation")
    .option(
      "--no-cleanup",
      "Keep installer archives + temp dirs after upgrade (debugging)",
    )
    .option(
      "--opencode-version <ver>",
    ...
    .action(async (opts) => {
      await upgradeEngineCommand(opts);
    });
  ...
  program
    ...
    .option(
      "--set-default",
      "Make the new model the default for opencode + launchers",
    )
  ...
  program
    ...
    .action(async (id, opts) => {
      await removeModelCommand(id, opts);
    });
  ...
  program
    ...
    .option(
      "--no-cleanup",
      "Keep installer archives + temp dirs after run (debugging)",
    )
    .action(async (list, opts) => {
      await addTargetsCommand(list, opts);
    });

  program
    .command("prune")
    .description(
      "Run ollama prune on the USB model store to reclaim orphaned blobs",
    )
  ...
  program
    .command("uninstall")
    .description(
      "Remove code-stick (binaries, models, config, launchers) from a USB",
    )
    .option("-t, --target <path>", "Uninstall from this directory")
    .option("-y, --yes", "Skip confirmation prompt")
    .option(
      "--force",
      "Proceed even if no code-stick manifest is detected at the target",
    )
    .action(async (opts) => {
...
main().catch((err) => {
  ...
  if (
    process.env.CODE_STICK_DEBUG === "1" &&
