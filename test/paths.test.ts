...
describe("safeJoin", () => {
  ...
  it("rejects ../ traversal", () => {
    expect(() => safeJoin("/tmp/parent", "../escaped")).toThrow(
      /Refusing to write outside/,
    );
  });

  it("rejects absolute child paths", () => {
    expect(() => safeJoin("/tmp/parent", "/etc/passwd")).toThrow(
      /Refusing to write outside/,
    );
  });
...
describe("toLongPath", () => {
  ...
  it("converts UNC paths into \\\\?\\UNC\\ form", () => {
    vi.spyOn(process, "platform", "get").mockReturnValue("win32");
    expect(toLongPath("\\\\server\\share\\file")).toBe(
...
describe("usbPaths", () => {
  it("derives canonical layout paths from a root", () => {
    ...
    expect(p.engine("linux-x64")).toBe(
      path.join("/mnt/usb", "engine", "linux-x64"),
    );
    expect(p.opencode("darwin-arm64")).toBe(
      path.join("/mnt/usb", "opencode", "darwin-arm64"),
    );
    expect(p.launcher("windows")).toBe(
      path.join("/mnt/usb", "start-windows.bat"),
    );
